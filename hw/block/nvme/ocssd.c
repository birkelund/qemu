...
static uint16_t ocssd_dsm(OcssdCtrl *o, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if (dsm->attributes & NVME_DSMGMT_AD) {
        ...
        for (int i = 0; i < nr; i++) {
            int64_t sidx = _sectr_idx(o, ons, range[i].slba);
...
static uint16_t ocssd_reset(OcssdCtrl *o, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    for (int i = 0; i < nlb; i++) {
        ...
        uint64_t sidx = _sectr_idx(o, ons, slba);
    ...
    Error **errp)
{
    ...
    id_ns->lbaf[0].ds = 63 - clz64(o->hdr.sector_size);
    ...
    ns_blks = ocssd_ns_calc_blks(o, ons);
...
static int ocssd_init_blk(OcssdCtrl *o, Error **errp)
{
    ...
    hdr = (OcssdFormatHeader) {
        ...
        .num_ns      = cpu_to_le32(n->num_namespaces),
    ...
    nblks = 1 << (lbaf.grp_len + lbaf.pu_len + lbaf.chk_len +
        lbaf.sec_len);
    fake_size = sizeof(OcssdFormatHeader) + n->num_namespaces * (
...
static void ocssd_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    if (n->num_namespaces != o->hdr.num_ns) {
        error_setg(errp, "geometry changed; invalid number of namespaces");
        return;
    }

    if (n->params.ms != o->hdr.md_size) {
        error_setg(errp, "geometry changed; invalid metadata bytes");
        return;
    }

    if (1 << o->params.lbads != o->hdr.sector_size) {
        error_setg(errp, "geometry changed; invalid sector size");
        return;
    }

    if (o->params.pe_cycles != o->hdr.pe_cycles) {
        error_setg(errp, "geometry changed; invalid number of p/e cycles");
        return;
    }

    n->num_namespaces = o->hdr.num_ns;
    n->params.ms = o->hdr.md_size;

    if (ocssd_init_namespaces(o, &local_err)) {
        error_propagate_prepend(errp, local_err, "ocssd_init_namespaces: ");
        return;
    }
