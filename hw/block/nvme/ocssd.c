 ...
 *
 *   -blockdev raw,node-name=ocssd01,file.driver=file,file.filename=ocssd.img
 *   -device ocssd,drive=ocssd01,serial=deadbeef,id=ocssd01
 *
 * In addition to the standard nvme device parameters, the following are also
 * available.
 *
 ...
 * image initialization time. If the image has already been initialized, the
 * values are fixed to what was specified when the image was first initialized.
 * If any of those parameters are specified and differs from what is stored in
 * the image, the device will error out and refuse to be realized.
 *
 ...
 *   ns=1 group=0 punit=3 chunk=0 prob=90
 *
 *
...
static inline bool _nsid_is_valid(OcssdCtrl *o, uint32_t nsid)
{
    if (unlikely(nsid == 0 || nsid > o->hdr.num_ns)) {
        return false;
    }
...
static uint16_t ocssd_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if (!_nsid_is_valid(o, nsid)) {
        trace_nvme_err_invalid_ns(nsid, o->hdr.num_ns);
        return NVME_INVALID_NSID | NVME_DNR;
    }
    ...
    Error **errp)
{
    ...
    if ((params->geometry.num_grp != id->geo.num_grp) ||
        (params->geometry.num_pu  != id->geo.num_pu)  ||
        (params->geometry.num_chk != id->geo.num_chk) ||
        (params->geometry.clba    != id->geo.clba)) {
        error_setg(errp, "geometry has changed; invalid image");
        return 1;
    }
...
static int ocssd_init_blk(OcssdCtrl *o, Error **errp)
{
    NvmeCtrl *n = &o->nvme;
    ...
    /*
     * The ocssd format begins with a 4k header. Namespaces are laid out
    ...
    usable_size = secs_total * (sec_size + n->params.ms);
    ...
    hdr = (OcssdFormatHeader) {
        ...
        .num_ns      = cpu_to_le32(n->params.num_ns),
        .md_size     = cpu_to_le32(n->params.ms),
        .sector_size = cpu_to_le64(sec_size),
        .ns_size     = cpu_to_le64(ns_size),
        .pe_cycles   = cpu_to_le32(o->params.pe_cycles),
        .lbaf        = lbaf,
    };
    ...
    for (int i = 0; i < n->params.num_ns; i++) {
        id = (OcssdIdentity) {
            ...
            .ver.minor = 0,
            .lbaf = lbaf,
            .mccap = cpu_to_le32(o->params.mccap),
            .wit = o->params.wit,
            .geo = (OcssdIdGeo) {
            ...
            .wrt = (OcssdIdWrt) {
                .ws_min    = cpu_to_le32(o->params.wdr.ws_min),
                .ws_opt    = cpu_to_le32(o->params.wdr.ws_opt),
                .mw_cunits = cpu_to_le32(o->params.wdr.mw_cunits),
            },
            .perf = (OcssdIdPerf) {
    ...
    nblks = 1 << (lbaf.grp_len + lbaf.pu_len + lbaf.chk_len +
        lbaf.sec_len);
    fake_size = sizeof(OcssdFormatHeader) + n->params.num_ns * (
        sizeof(OcssdIdentity) + info_size + acct_size +
        nblks * (sec_size + n->params.ms));
...
static void ocssd_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    if (!blk_getlength(n->conf.blk)) {
        ret = ocssd_init_blk(o, &local_err);
        if (ret < 0) {
            error_propagate_prepend(errp, local_err, "ocssd_init_blk: ");
            return;
        }
    }
    ...
    if ((n->params.num_ns     != o->hdr.num_ns)      ||
        (n->params.ms         != o->hdr.md_size)     ||
        (1 << o->params.lbads != o->hdr.sector_size) ||
        (o->params.pe_cycles  != o->hdr.pe_cycles)) {
        error_setg(errp, "geometry has changed; invalid image");
        return;
    }
